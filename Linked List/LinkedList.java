...
public class LinkedList {
    ...
    public void removeAfter(int n){
        ...
        while(current != null) {
            //If the current node is the node we are looking for deletes the next node otherwise we move on to the next node
    ...
    public LinkedList copy(){
        ...
        //Iterates through the LinkedList to copy each node
        while(original!= null){
    ...
    public int max(){
        ...
        Node current = head;
        if(current != null){
            //Sets max to the current data
            int max = current.getData();
            //Iterates through
            while(current.getNext()!= null){
                current = current.getNext();
                //Checks if the value of the node is greater than max
                if(max< current.getData())
                    max = current.getData();
            }
            return max;
        }
        //If the LinkedList has no nodes returns 0
        else
            return 0;
    }
